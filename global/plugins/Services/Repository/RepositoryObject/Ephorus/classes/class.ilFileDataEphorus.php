...
/**
* This class handles all operations on files for the ephorus object
*
* @author	Stefan Meyer <meyer@leifos.com>
* @version $Id: class.ilFileDataEphorus.php 23386 2010-03-26 06:47:42Z akill $
*
* @ingroup	ModulesEphorus
*/
class ilFileDataEphorus extends ilFileData
{
	/**
	* obj_id
	* @var integer obj_id of ephorus object
	* @access private
	*/
	var $obj_id;

	/**
	* path of ephorus directory
	* @var string path
	* @access private
	*/
	var $ephorus_path;
	...
	* @access	public
	*/
	function ilFileDataEphorus($a_obj_id = 0)
	{
die ("ilFileDataEphorus is deprecated.");
		define('EXERCISE_PATH','ephorus');
		parent::ilFileData();
		$this->ephorus_path = parent::getPath()."/".EXERCISE_PATH;
	...
	* @return string path
	*/
	function getEphorusPath()
	{
		return $this->ephorus_path;
	}

	function getFiles()
	{
		$files = array();
		$dp = opendir($this->ephorus_path);
		...
		while($file = readdir($dp))
		{
			...
			{
				if(!is_dir($this->ephorus_path.'/'.$file))
				{
					$files[] = array(
						'name'     => $rest,
						'size'     => filesize($this->ephorus_path.'/'.$file),
						'ctime'    => ilFormat::formatDate(date('Y-m-d H:i:s',filectime($this->ephorus_path.'/'.$file))),
						'fullpath' => $this->ephorus_path.'/'.$file);
				}
			}
		}
		closedir($dp);
	...
	function ilClone($a_new_obj_id)
	{
		...
		{
			@copy($this->getEphorusPath()."/".$this->obj_id.'_'.$file["name"],
				  $this->getEphorusPath()."/".$a_new_obj_id.'_'.$file["name"]);
		}
		return true;
	}
	function delete()
	{
		...
		$delivered_file_path = $this->getEphorusPath() . "/" . $this->obj_id . "/";
	...
	*/
	function storeUploadedFile($a_http_post_file, $secure_filename = false, $is_unziped = false)
	{
		...
		if(isset($a_http_post_file) && $a_http_post_file['size'])
		{
			...
			$this->__rotateFiles($this->getEphorusPath().'/'.$this->obj_id.'_'.$filename);
			//move_uploaded_file($a_http_post_file['tmp_name'],$this->getEphorusPath().'/'.$this->obj_id.'_'.
			//				   $filename);
			
			if (!$is_unziped)
			{
				ilUtil::moveUploadedFile($a_http_post_file['tmp_name'], $a_http_post_file['name'],
				$this->getEphorusPath().'/'.$this->obj_id.'_'.$filename);
			}
			else
			{
	...
	*/
	function downloadAllDeliveredFiles($members)
	{
		...
		$ephorusTitle = $ilObjDataCache->lookupTitle($this->getObjId());
		ilUtil::deliverFile($tmpzipfile, (strlen($ephorusTitle) == 0? strtolower($lng->txt("ephs")) : $ephorusTitle). ".zip");
		chdir($cdir);
		unlink($tmpfile);
		unlink($tmpzipfile);
	}
	...
	*/
	...
	{
		if(file_exists($this->ephorus_path.'/'.$this->obj_id.'_'.$a_filename))
		{
			return unlink($this->ephorus_path.'/'.$this->obj_id.'_'.$a_filename);
		}
	}
	/**
	...
	*/
	...
	{
		return $this->ephorus_path.'/'.$this->obj_id.'_'.$a_path;
	}
	...
	*/
	function checkFilesExist($a_files)
	{
		if($a_files)
		{
			...
			{
				if(!file_exists($this->ephorus_path.'/'.$this->obj_id.'_'.$file))
				{
					return false;
				}
			}
	...
	{
		if(!@file_exists($this->getEphorusPath()))
		{
			return false;
		}
	...
	*/
	...
	{
		if(is_writable($this->ephorus_path) && is_readable($this->ephorus_path))
		{
			return true;
		}
		else
		{
			$this->ilias->raiseError("Ephorus directory is not readable/writable by webserver",$this->ilias->error_obj->FATAL);
		}
	}
	/**
	* init directory
	...
	*/
	function __initDirectory()
	{
		...
		{
			return ilUtil::makeDir($this->ephercise_path = $this->getPath().'/'.EXERCISE_PATH);
		}
		return false;
	}
	/**
